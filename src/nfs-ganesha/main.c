...
static fsal_status_t lzfs_fsal_create_export(struct fsal_module *fsal_hdl,
                                             void *parse_node,
                                             ...
                                             const struct fsal_up_vector *up_ops)
{
    ...
    if (parse_node) {
        rc = load_config_from_node(parse_node, &lzfs_fsal_export_param_block,
                                   lzfs_export, true, err_type);
		if (rc != 0) {
            LogCrit(COMPONENT_FSAL, "Failed to parse export "
                    "configuration for %s", CTX_FULLPATH(op_ctx));
...
static fsal_status_t lzfs_fsal_init_config(struct fsal_module *module_in,
                                           ...
                                           struct config_error_type *err_type)
{
    struct lzfs_fsal_module *lzfs_module = NULL;
